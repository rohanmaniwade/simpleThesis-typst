...
The damage rate is estimated using Rice's formula @Rice1944 for narrowband processes (irregularity factor $y approx 1$),
...
where $ Q=1/(2 zeta) $ is the quality factor, $Gamma$ is the gamma function, and $T_(e q)$ is the equivalent test duration. This inversion assumes narrowband response (Rayleigh damage model) and provides a lower-bound PSD estimate.

=== Implementation of an iterative method
While the Lalanne formula provides an excellent initial estimate, it assumes narrowband response. In practice, structural vibration may have broadband characteristics. Therefore, an iterative refinement process is employed.
