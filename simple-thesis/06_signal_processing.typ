...
6. Apply time reduction from @timecompression to derive accelerated PSD for 1 h, 1.5 h, 2 h, 4h, 5 h, 10 h, 20 h, 50 h, and 100 h.
...
Additionally, _VibeAccelKit_ incorporates validation capabilities to ensure that the accelerated test specifications do not exceed critical response limits, such as the SRS and ERS, thereby preventing overtesting.

Built upon _NumPy_ @harris2020 and _SciPy_ @virtanen2020 _VibeAccelKit_ implements both time domain and frequency domain approaches for FDS calculation. The time domain method employs a SDOF oscillator simulation coupled with rainflow cycle counting to directly quantify fatigue damage from accelerated time histories. The frequency domain approach leverages spectral moment analysis and probabilistic damage rate models (_Rice_ @Rice1944  and _Rayleigh_ @rayleigh1880 distributions) to compute FDS directly from PSD, offering computational advantages for stationary random processes.

The core acceleration methodology follows _Lalanne’s_ analytical framework, which relates FDS to an equivalent PSD through the relationship between damage accumulation, spectral density, and test duration. The acceleration process scales the equivalent PSD according to the target test duration and fatigue exponent, maintaining damage equivalence while reducing test time. Validation routines that verify that the accelerated profile satisfies design constraints, ensuring that response spectra remain within acceptable bounds relative to the original mission requirements. 
...
With time histories collected for each operating mode of _Thermomix#super[#sym.trademark.registered] TM7_, _Lalanne’s_ specification was applied to derive accelerated PSD that are damage equivalent to the field profile while compressing duration. The _Python_ library _VibeAccelKit_ @vibeaccelkit2025 used for signal processing, test synthesis, and verification.


== Data import and preprocessing

The recorded vibration time histories for all speed modes were imported as `.csv` files for processing. _ArtemiS Suite_ natively records measurements in `.hdf` format, which was converted to `.csv` within the software before export. Each `.csv` file contains time history data for all nine channels, corresponding to the three accelerometers measuring along their respective X, Y, and Z axes.
...
@timehistory_center_z presents the vibration time histories recorded at sensor `PCB_CENTER` along the Z axis for all operating modes and shock events. Most speed modes exhibit stationary random behaviour, with relatively stable amplitude distributions over time. The dough mode introduces transient variations due to the alternating blade direction and dough resistance. The signal in black corresponds to the cheese block blending event, where two 200 gram blocks of _Grana Padano_ were processed at Speed 8 (5800 rpm). This event produced significantly higher amplitude peaks and broader frequency content compared to normal operation, making it suitable as a reference shock for validating that accelerated profiles do not exceed realistic severity limits.
...
For frequency domain analysis, RMS is computed from the PSD using _Parseval's theorem_ @BendatPiersol2010,

$ x_(R M S)= sqrt(integral^infinity_0 S_(x x)(f)d f) $ <rmspsdcalculation>

where $S_(x x)(f)$ is the power spectral density. _VibeAccelKit_ implements using the trapezoidal rule for improved accuracy over discrete frequency grids.

== Power Spectral Density estimation

The PSD characterises the frequency content and intensity of random vibration signals @BendatPiersol2010 @Newland2012. _VibeAccelKit_ implements _Welch's_ method @Welch1967 for robust PSD estimation from time histories,

$ S_(x x)(f)= 1/K sum^K_(k=1)|(F F T_k (x)|^2 dot 1/(f_s.N_(s e g)) $ <psdfromtimehistory>

where $K$ is the number of overlapping segments, $N_(s e g)$ is the segment length, and $f_s$ is the sampling frequency. The method applies a _Hanning_ window to each segment to reduce spectral leakage,
...
_MIL-STD-810_ random vibration profiles for electronics typically span 20-2000 Hz. Extending the analysis beyond 2 KHz does not improve fidelity for _PCB_-level durability assessments, since the standard environmental spectra contain negligble energy above this point @MILSTD810H_2019. 
...
The damage rate is estimated using _Rice's formula_ @Rice1944 for narrowband processes (irregularity factor $y approx 1$),

$ dot(D)_(R a y l e i g h)= v_p sigma^b dot 2^(b/2) Gamma (1+b/2) $

where $v_p = sqrt(m_2/m_0)$ is the peak frequency and $sigma=sqrt(m_o)$ is the RMS response. For broadband processes, _Rice's_ general formulation is employed @Rice1944 @Rice1945,
...
Finally, FDS of multiple mission profiles are combined using time-weighted summation giving us the composite FDS @MILSTD810H_2019 @lalanne2010mechanicalvol,
...
#figure(
  ...
  caption: [Initial equivalent PSD derived from _Lalanne's formula_ for `PCB_CENTER` Z-axis ($b=9$, $zeta=0.05$, $T_(e q)=924$ hours)]
)<psdinit>

@psdinit shows the initial equivalent PSD obtained by directly applying _Lalanne's_ analytical inversion formula per @fdstopsdinit to the composite FDS. This initial estimate assumes narrowband _Rayleigh_ response characteristics and provides the starting point for the iterative refinement process. While this analytical solution offers a computationally efficient first approximation, the assumption of narrowband behaviour may not fully capture the broadband nature of the actual structural response.
...
#figure(
  ...
  caption: [Comparison of initial and converged equivalent PSD for `PCB_CENTER` Z-axis]
)<psdmatch>

The iterative refinement process adjusts the initial PSD to account for broadband response characteristics using _Rice's_ more general damage formulation. @psdmatch compares the initial PSD from _Lalanne's formula_ with the converged PSD after iterative correction. At each iteration, the multiplicative correction factor from @corr is applied frequency-by-frequency to adjust the PSD based on the ratio of target to computed FDS values. The refined profile shows localised adjustments, particularly in frequency regions where the narrowband assumption deviates from the actual broadband damage accumulation. Under-relaxation and gain limiting ensure stable convergence without oscillation.
...
caption: [PSD envelope smoothing],
)<psdsmoothing>
...
The SRS represents the maximum absolute acceleration response of an SDOF oscillator subjected to the excitation. The calculation of SRS is explained in @srs. The maximax SRS convention is adopted in this work, meaning the peak of peak values across events @lalanne2014_mechanical_shock. For numerical implementation, the well established recursive algorithm of _Smallwood_ (1981) @smallwood1981_improved_srs is used, which provides an efficient and stable way to compute the SRS for arbitrary time histories. @srsenvelopecheese shows the SRS of all the channels and axes of the shock event, which is the blending of 400 grams of _Grana Padano_ cheese in _Thermomix#super(sym.trademark.registered) TM7_.

=== Extreme Response Spectrum calculation

The ERS is used to estimate the expected peak absolute acceleration response of a family of base excited SDOF oscillators subjected to stationary random vibration. The calculation of ERS in time domain is explained in @ers. However, for stationary Gaussian random signals, the ERS can be computed from PSD.
...
The expected peak is obtained using _Vanmarcke's_ peak factor model @vanmarcke1972_spectralmoments,
...
The validation follows the procedure described in @responsespectrumvalidation. To avoid overtesting, the following validation rule is adopted,

$ E R S_(a c c)(f_0) <= S R S(f_0) $

The reference SRS envelope is constructed from the shock event recorded during data acquisition, which is the cheese block blending event. This event represents the most severe transient load the backend experiences during operation. The SRS is computed using the maximax convention, taking the peak absolute acceleration response at the same damping ratio and frequency grid as the ERS calculation.
...
#figure(
  ...
  caption: [PSD of shock event - Blending of 400 grams of _Grana Padano_]
)
...
#figure(
  ...
  caption: [SRS Envelope of Blending of _Grana Padano_]
) <srsenvelopecheese>
