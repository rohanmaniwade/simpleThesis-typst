...
The behaviour at the `PCB_LEFT` position is expected to have a close agreement because the controller continuously adjusts the drive signal to keep the response at `PCB_LEFT` close to the target profile. The FDS at that location therefore follows the composite and the mission acceleration curves very closely. The `PCB_CENTER` and `PCB_TOP` positions, however, are not controlled and their response depends on the natural dynamic properties of the PCB and the fixture. The FDS reveal how the accelerated profile propagates across the assembly rather than only at the control point.
