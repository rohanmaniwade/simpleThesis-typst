...
The important point is that these variations remain within a narrow and acceptable range and the dominant fatigue carrying frequencies are reproduced correctly. The overall shape of the damage distribution is preserved and the cumulative demands of the mission are matched. This behaviour indicates that the accelerated profile excites the correct structural modes and that the test remains representative across the entire PCB board, even at the positions that are not part of the control loop.
...
Some response channels, however, show narrow frequency bands in which the ERS exceeds the real shock envelope. This behaviour was observed at the `PCB_CENTER` Z-axis and at the `PCB_TOP` X and Z axes for both values of $b=8$ and $b=9$. These exceedances occur close to the principal modes of the backend PCB where the structure has high dynamic amplification. The resulting responses therefore rise above what would normally be considered acceptable for a perfect agreement with the real shock envelope. Even though they do not fall within the ideal limits, the deviations are narrow in bandwidth, limited in magnitude and fully explain modal amplification and fixture transmissibility. They do not indicate unsafe loading and do not affect overall physical representativeness of the accelerated test.
...
The use of two values of the fatigue exponent, $b=8$ and $b=9$, follows the recommendation given for electronic assemblies by Christian Lalanne in his work Specification Development @lalanne2010mechanicalvol. In principle, the exponent influences the balance between broadband and peak dominated contributions to fatigue damage. In the present work, the differences between the two values are modest. The backend PCB is strongly governed by a small number of dominant modes and these modes control the fatigue behaviour of both exponents. As a result the synthesised profiles, the FDS and the MRS obtained for both values of fatigue exponent are very similar. The final test durations of 4 hours and 1.5 hours are mainly determined by the limits on test compression and on the allowable exaggeration of the MRS rather than by a strong sensitivity to the exponent itself. This indicates that the methodology is robust with respect to the exact choice of $b$ within the range advised for electronic components.
