...
Some response channels, however, show narrow frequency bands in which the ERS exceeds the real shock envelope. This behavior was observed at the `PCB_CENTER` Z-axis and at the `PCB_TOP` X and Z axes for both values of $b=8$ and $b=9$. These exceedances occur close to the principal modes of the backend PCB where the structure has high dynamic amplification. The resulting responses therefore rise above what would normally be considered accceptable for a perfect agreement with the real shock envelope. Even though they do not fall within the ideal limits, the deviations are narrow in bandwidth, limited in magnitude and fully explain modal amplification and fixture transmissibility. They do not indicate unsafe loading and do not affect overall physical representativeness of the accelerated test.
